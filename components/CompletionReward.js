...
export default function CompletionReward({ gameState, showManually = false }) {
  ...
  const closeModal = () => {
    ...
    document.body.style.overflow = "auto"; // Ensure scrolling is enabled when closing
  }
  
  if (!allCompleted) return null;
  
  return (
    <div className={`fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-70 transition-opacity duration-1000 overflow-y-auto p-4 ${showAnimation ? 'opacity-100' : 'opacity-0'}`}>
      <div className={`bg-white rounded-xl border-4 border-pink-400 p-6 max-w-2xl transition-all transform duration-1000 max-h-[90vh] overflow-y-auto ${showAnimation ? 'scale-100 opacity-100' : 'scale-90 opacity-0'}`}>
